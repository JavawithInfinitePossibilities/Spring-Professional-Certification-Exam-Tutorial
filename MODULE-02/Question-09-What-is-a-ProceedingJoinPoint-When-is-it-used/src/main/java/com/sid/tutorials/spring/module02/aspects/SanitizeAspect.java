package com.sid.tutorials.spring.module02.aspects;
...
public class SanitizeAspect {
	...
	private boolean containsAnnotationOfType(Annotation[] parameterAnnotations, Class sanitizeClass) {
		return Arrays.stream(parameterAnnotations).map(Annotation::annotationType).anyMatch(sanitizeClass::equals);
	}
}
