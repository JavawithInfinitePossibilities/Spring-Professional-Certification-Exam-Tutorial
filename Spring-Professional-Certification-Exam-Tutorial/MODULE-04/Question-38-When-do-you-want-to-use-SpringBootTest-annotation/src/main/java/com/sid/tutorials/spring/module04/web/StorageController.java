...
public class StorageController {
	...
	public ResponseEntity<String> fetchFromDb(@PathVariable int id) {
		return dbStorageService.fetch(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
	}

	@GetMapping("db/storage/")
	public ResponseEntity<String> fetchFromDb() {
		return dbStorageService.fetch().map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
	}
	...
	public ResponseEntity<String> fetchFromFile(@PathVariable int id) {
		return fileStorageService.fetch(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
	}
}
